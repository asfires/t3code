...
} from "./components/composerInlineTokenPaste";
...
describe("pasted-text chips in the Tiptap document", () => {
  it.each([false, true])(
    "preserves chip identity and payload through copy and restore, rich=%s",
    (rich) => {
      const draft = {
        id: "paste",
        text: "**literal** pasted contents\n".repeat(30),
        createdAt: "2026-09-18T12:00:00.000Z",
      };
      const record = pastedTextContextRecord(draft);
      const reference = formatInlineContextReference(pastedTextContextReference(draft));
      const value = `**before** ${reference} after`;
      const json = buildDocJson(value, (name) => ({ label: name, description: null }), {
        styling: rich,
      });
      const doc = ProseMirrorNode.fromJSON(rich ? schema : plainSchema, json);
      const chips: string[] = [];
      doc.descendants((node) => {
        if (node.type.name === "composer-context-reference") chips.push(node.attrs.contextId);
      });
      expect(chips).toEqual([record.contextId]);
      const serialized = serializeEditorDoc(doc);
      expect(serialized.value).toBe(value);
      expect(serialized.contextIds).toEqual([record.contextId]);
      const fragment = encodeComposerContextFragment({
        version: 1,
        source: { environmentId: EnvironmentId.make("test") },
        records: [record, pastedTextContextRecord({ ...draft, id: "unselected" })],
      })!;
      for (const flavor of ["custom", "html"]) {
        const clipboard = {
          getData: (mime: string) =>
            mime === "text/plain"
              ? serialized.value
              : mime === COMPOSER_CONTEXT_CLIPBOARD_MIME && flavor === "custom"
                ? fragment
                : mime === "text/html" && flavor === "html"
                  ? encodeComposerContextClipboardHtml(serialized.value, fragment)
                  : "",
        };
        expect(readPastedComposerContext(clipboard)?.records).toEqual([record]);
        const copied = importPastedComposerText(clipboard, (imported) => {
          expect(imported.records).toEqual([record]);
          return new Map([[record.contextId, "pasted-text_restored"]]);
        });
        const restored = (rich ? roundTrip : roundTripPlain)(copied);
        expect(restored.contextIds).toEqual(["pasted-text_restored"]);
        const projected = projectComposerContextForProvider({
          text: restored.value,
          records: [pastedTextContextRecord({ ...draft, id: "restored" })],
        });
        expect(projected).toContain(draft.text.trim());
