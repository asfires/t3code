...
import ChatMarkdown, { ChatMarkdownAssetImage, reportMarkdownActionFailure } from "../ChatMarkdown";
...
function TimelineMinimap({
  ...
  const minimapButtonRef = useRef<HTMLDivElement | null>(null);
  ...
  return (
    <div className="border-b border-border/60 pb-2 pt-1">
      <div className="flex h-6 min-w-0 items-baseline gap-2 px-1 text-sm leading-relaxed text-muted-foreground tabular-nums">
        ...
        {workingStepLabel && !isCompacting && !isPreparingWorktree ? (
          <span className="ml-2 truncate">{workingStepLabel}</span>
        ) : null}
        {backgroundWorktreeSetup ? (
...
const toolCallExpandedBodyClassName =
  "max-h-64 cursor-text overflow-auto whitespace-pre-wrap break-words font-mono text-secondary-label text-[length:var(--font-size-tool-output,0.6875rem)] leading-relaxed select-text";
...
const PlainWorkEntryRow = memo(function PlainWorkEntryRow(props: {
  ...
  const entryToolIcon = showWarningIndicator
    ? undefined
    : (workEntry.toolIcon ?? workEntry.toolSource?.icon);
  // The row splits the tool heading from its argument preview so the two read
  // as separate columns. A standalone edit keeps the group-style summary
  // ("Changed 1 file") as its heading, with the path as the preview.
  const entryText = workEntryDisplayText(workEntry, workspaceRoot);
  const heading =
    !isExpandedToolGroupEntry && toolGroupAction(workEntry) === "edit"
      ? summarizeToolGroup([workEntry])
      : entryText.heading;
  const { preview, displayText } = entryText;
  ...
  const viewedImage =
    viewedImagePath && threadRef
      ...
      : null;
  // Built eagerly (memoized per entry) so the chevron and copy control know
  // whether there is command output to reveal before the row is opened.
  const expandedBody = useMemo(
    ...
    [displayText, viewedImage, viewedImagePath, workEntry, workspaceRoot],
  );
  // A clipped preview is itself worth expanding: opening the row unfurls it
  // in place, and any output or detail blocks render underneath.
  const canExpand =
    Boolean(workEntry.questionAnswer) ||
    expandedBody !== null ||
    viewedImage !== null ||
    preview !== null;
  const previewUnfurled = preview !== null && expanded;
  ...
  return (
    <div
      className={cn(
        "group/work-entry group/timeline-row relative flex flex-col rounded-md px-0.5 transition-colors",
    ...
    >
      <div
        className={cn(
          ...
          previewUnfurled ? "items-start" : "items-center",
        )}
      >
        <span
          className={cn(iconWrapperClass, !showEntryIcon && "invisible")}
          aria-hidden={!showEntryIcon}
          role={showEntryIcon && showFailedIndicator ? "img" : undefined}
          aria-label={showEntryIcon && showFailedIndicator ? "Tool call failed" : undefined}
        >
          <ToolActivityIconView
            icon={entryToolIcon}
            ...
            <p className="flex min-w-0 w-full items-baseline gap-1.5 text-[12px] leading-5">
              ...
              {preview && (
                <span
                  ...
                  onClick={previewUnfurled ? stopRowToggleWhileSelectingText : undefined}
                  onPointerDown={previewUnfurled ? stopRowToggle : undefined}
                >
                  {previewJson ? (
                    <>
      ...
      {expanded && expandedBody && !workEntry.questionAnswer ? (
        <WorkEntryExpandedBody body={expandedBody} />
      ) : null}
    </div>
  );
});
