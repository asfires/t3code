...
const SimpleWorkEntryRow = memo(function SimpleWorkEntryRow(props: {
  ...
  const { rowId, workEntry, workspaceRoot, isExpandedToolGroupEntry, displayLabel } = props;
...
const PlainWorkEntryRow = memo(function PlainWorkEntryRow(props: {
  ...
  const { rowId, workEntry, workspaceRoot, isExpandedToolGroupEntry, displayLabel } = props;
  const { threadRef, onImageExpand, onToggleWorkEntry } = use(TimelineRowCtx);
  ...
  const toggleExpanded = () => {
    const next = !expanded;
    if (rowId) onToggleWorkEntry(rowId);
    if (groupView) {
  ...
  const headingClass = showWarningIndicator
    ...
    : showDestructiveRowStyle
      ...
      : workLogEntryIsToolLike(workEntry)
        ? "text-secondary-label"
        : "text-foreground/80";
  const commandUnfurled = expanded && commandMatchesVisibleLabel;
  ...
  return (
    <div
      className={cn(
        "group/work-entry flex flex-col rounded-md px-0.5 transition-colors",
    ...
    >
      ...
      {expanded && canExpand && expandedBody ? <WorkEntryExpandedBody body={expandedBody} /> : null}
    </div>
  );
});
