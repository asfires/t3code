...
const make = Effect.gen(function* () {
  ...
  const ensureThreadWorktree = Effect.fnUntraced(function* (thread: {
    ...
    readonly managedWorktree?: unknown;
  }) {
    const { worktreePath, branch } = thread;
    // Worktrees created by first-send bootstrap carry provenance and are
    // recreated by ensureThreadWorkspaceCwd, which can also recover a pruned
    // branch from refs or checkpoints. Only unmanaged worktrees use this
    // best-effort path.
    if (!worktreePath || !branch || thread.managedWorktree != null) {
      return;
    }
  ...
  const ensureThreadWorkspaceCwd = Effect.fn("ensureThreadWorkspaceCwd")(function* (input: {
    readonly thread: Pick<
      OrchestrationThread,
      "id" | "projectId" | "worktreePath" | "managedWorktree" | "branch"
    >;
    ...
    const recoveryPlan = managedWorktreeRecoveryPlan({
      ...
      refs: refs.refs,
      checkpoints: (yield* resolveThreadDetail(input.thread.id))?.checkpoints ?? [],
    });
