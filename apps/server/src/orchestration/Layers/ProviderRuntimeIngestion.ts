    ...
    Effect.gen(function* () {
      ...
      if (
        ...
        if (
          ...
          const taskActivities = Option.match(taskActivity, {
            onNone: () => undefined,
            onSome: (activity) => [activity],
          });
          resolvedTaskPayload = inheritTaskIdentityFromActivities(payload, taskActivities);
          taskTitle =
            taskTitle ??
            taskTitleFromPayload(resolvedTaskPayload) ??
            findTaskTitleInActivities(taskActivities, event.payload.taskId);
        }
      }
      ...
      const activities = runtimeEventToActivities(activityEvent, taskTitle, resolvedTaskPayload);
