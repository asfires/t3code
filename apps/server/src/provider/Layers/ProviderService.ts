...
const makeProviderService = Effect.fn("makeProviderService")(function* (
  ...
  const persistLiveResumeCursor = Effect.fn("ProviderService.persistLiveResumeCursor")(
    function* (input: {
      ...
      // A stale session on a replaced provider must not clobber the binding
      // that now names the thread's active provider.
      const binding = yield* directory.getBinding(input.threadId);
      if (
        Option.isSome(binding) &&
        (binding.value.provider !== input.adapter.provider ||
  ...
  const processRuntimeEvent = (
    ...
    Effect.gen(function* () {
      ...
      if (canonicalEvent.type === "turn.completed" && source.provider === "claudeAgent") {
