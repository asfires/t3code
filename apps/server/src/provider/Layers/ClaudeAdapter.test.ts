...
describe("ClaudeAdapterLive", () => {
  ...
  it.effect("serializes concurrent sends into one active turn per thread", () => {
    const harness = makeHarness();
    return Effect.gen(function* () {
      const adapter = yield* ClaudeAdapter;
      yield* adapter.startSession({
      ...
      // Permission-mode updates yield before the turn is registered. Concurrent
      // sends must see the first turn after that await and steer it.
      ...
      assert.equal((yield* adapter.listSessions())[0]?.activeTurnId, turns[0]?.turnId);
    }).pipe(
      Effect.provideService(Random.Random, makeDeterministicRandomService()),
      Effect.provide(harness.layer),
    );
  ...
  it.effect("interruptTurn settles every acknowledged live task before closing the session", () => {
    const harness = makeHarness();
    return Effect.gen(function* () {
      const adapter = yield* ClaudeAdapter;
      ...
      // The acknowledged task stop is retained even when the query closes.
